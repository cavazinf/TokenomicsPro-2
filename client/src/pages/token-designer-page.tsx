  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card>
                ...
                {formData.utility.length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {formData.utility.map((item: string) => (
                        <span key={item} className="bg-primary/10 text-primary px-2 py-1 rounded-md text-xs">
                          {utilityOptions.find(u => u.id === item)?.label}
                        </span>
                      ))}
                    </div>
                  </div>
        ...
        <CardContent>
          {/* This content area is intentionally left empty as the tabs content is now inside the Tabs component */}
        </CardContent>
      </Card>
    </div>
  );
